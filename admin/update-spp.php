...
$selectData = $stmt = $conn->prepare("SELECT * FROM spp WHERE id_spp = ?");
$selectData->bind_param("s",$id);
$selectData->bind_result($id_spp,$tahun,$nominal);
$query = $selectData->execute();

$row = $selectData->fetch();
...
<html lang="en">
  <head>
    ...
    <title>Update Data SPP - Aplikasi Pembayaran SPP</title>
  ...
  <main>
        ...
        if ($_SESSION['status'] == 'tersedia') {
            echo "<script>
                Swal.fire({
                  ...
                  text: 'Data tahun {$_POST['tahun']} sudah tersedia',
                  icon: 'warning',
   ...
   <section id="add-officer" class="mx-3">
    ...
    <h2 class="my-3">Update Data SPP</h2>



    <form action="" class="my-3" method="post">
    	<input type="hidden" value="<?= $id_spp ?>" name="id">


    	<label for="tahun" class="mb-2">Tahun</label>
    	<input type="number" value="<?= $tahun ?>" name="tahun" id="tahun" class="form-control mb-3" autocomplete="off" required>

      <input type="hidden"  value="<?= $tahun ?>" name="currentTahun">
      <label for="nominal" class="mb-2">Nominal</label>
      <input type="number" value="<?= $nominal ?>" name="nominal" id="nominal" class="form-control mb-3" autocomplete="off" required>

    	<a href="data-spp" class="btn btn-secondary">Kembali</a>
